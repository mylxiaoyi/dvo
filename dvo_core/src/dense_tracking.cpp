...
: cfg (cfg), weight_calculation_ (), itctx_ (cfg)
{
    std::cout << "DenseTracker ctor" << std::endl;
...
{
    last_xi_ = Sophus::SE3d (last_transformation.rotation (),
                             last_transformation.translation ());
}
                          ...
                          Eigen::Affine3d& transformation)
{
    ...
    for (itctx_.Level = cfg.FirstLevel; itctx_.Level >= cfg.LastLevel; --itctx_.Level)
    {
        ...
        do
        {
            ...
            computeLeastSquaresEquationsInverseCompositional (ref, cur, intrinsics,
                                                              estimate (), ls);

            itctx_.LastError = itctx_.Error;
            itctx_.Error = ls.error + 0.5 * cfg.Lambda * old ().log ().squaredNorm () +
                           0.5 * cfg.Mu * initial ().log ().squaredNorm ();
            ...
            // if we get a worse result, we revert the increment and try our
            // luck on the next pyramid level
            if (!accept)
            {
                old.revert ();
                initial.revert ();
                estimate.revert ();
                inc = Sophus::SE3d::exp (Vector6::Zero ().cast<double> ());
            ...
            //{
            //  Eigen::Vector3f rot = ls.A.bottomRightCorner(3,
            //  3).ldlt().solve(ls.b.tail(3));
            //  x.setZero();
            //  x.tail<3>() = rot;
            //}
            // else
            ...
            // ROS_DEBUG_STREAM_COND(accept, itctx_ << ", Increment: " <<
            // x.format(YamlArrayFmt));

            inc = Sophus::SE3d::exp (x.cast<double> ());

            itctx_.Iteration++;
            itctx_.NumConstraints = ls.num_constraints;

            sw_it[itctx_.Level].stopAndPrint ();
        } while (accept && itctx_.ErrorDiff () > cfg.Precision &&
                 !itctx_.IterationsExceeded ());

        sw_level[itctx_.Level].stopAndPrint ();
    }

    // if last increment wasn't incorporated because of iterations exceeded,
    // incorporate it
    ...
    // ROS_DEBUG_STREAM_COND_NAMED(!itctx_.IterationsExceeded() &&
    // itctx_.ErrorDiff() > 0.0 && itctx_.ErrorDiff() <= cfg.Precision, "abort",
    // "error_precision");
    // ROS_DEBUG_STREAM_COND_NAMED(!itctx_.IterationsExceeded() &&
    // itctx_.ErrorDiff() < 0.0, "abort", "error_increase");
    // ROS_DEBUG_STREAM_COND_NAMED(itctx_.IterationsExceeded(), "abort",
    // "iteration_exceeded");

    if (success)
    {
        last_xi_ = Sophus::SE3d (estimate ().rotation ().cast<double> (),
...
dvo::core::LeastSquaresInterface& ls)
{
    // static stopwatch_collection sw_warp(5, "warp@l"), sw_eqs(5, "eqs@l"),
    // sw_total(5, "total@l");
...
struct LeastSquaresEquationsReduction
{
    ...
    LeastSquaresEquationsReduction (const cv::Mat& residuals,
                                    const dvo::core::WeightCalculation& weights,
                                    const cv::Mat& Jix,
                                    const cv::Mat& Jiy,
                                    const dvo::core::RgbdImage::PointCloud& points)
    : ls (new dvo::core::NormalEquationsLeastSquares ()), residuals (residuals),
      weights (weights), Jix (Jix), Jiy (Jiy), points (points)
    {
        ls->initialize (1);
    }
    ...
    : ls (new dvo::core::NormalEquationsLeastSquares ()),
      residuals (other.residuals), weights (other.weights), Jix (other.Jix),
      Jiy (other.Jiy), points (other.points)
    {
        ls->initialize (1);
    }
    ...
    {
        const float *jix_ptr = Jix.ptr<float> () + r.begin (),
                    *jiy_ptr = Jiy.ptr<float> () + r.begin (),
                    *residual_ptr =
                    residuals.ptr<float> () + r.begin (); //, *weights_ptr =
                                                          //weights.ptr<float>()
                                                          //+ r.begin();
        Matrix1x2 Ji;
        Matrix2x6 Jw;

        dvo::core::NormalEquationsLeastSquares* tmp =
        ls; // new dvo::core::NormalEquationsLeastSquares();

        for (size_t idx = r.begin (); idx != r.end ();
             ++idx, ++jix_ptr, ++jiy_ptr, ++residual_ptr) //, ++weights_ptr)
        {
            if (!isfinite (*jix_ptr) || !isfinite (*jiy_ptr) || !isfinite (*residual_ptr))
                continue;
...
inline void
DenseTracker::computeLeastSquaresEquationsGeneric (const cv::Mat& residuals,
                                                   const cv::Mat& Jix,
                                                   const cv::Mat& Jiy,
                                                   const dvo::core::RgbdImage::PointCloud& points,
                                                   dvo::core::LeastSquaresInterface& ls)
{
    // static stopwatch_collection sw_weights(5, "weights@l"), sw_reduce(5,
    // "reduce@l");
