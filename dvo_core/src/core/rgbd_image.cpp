...
static void pyrDownMeanSmooth (const cv::Mat& in, cv::Mat& out)
{
    ...
    for (int y = 0; y < out.rows; ++y)
    {
        for (int x = 0; x < out.cols; ++x)
        {
            ...
            out.at<T> (y, x) = (T) ((in.at<T> (y0, x0) + in.at<T> (y0, x1) +
                                     in.at<T> (y1, x0) + in.at<T> (y1, x1)) /
                                    4.0f);
        }
    }
}
...
RgbdImagePyramid::RgbdImagePyramid (cv::Mat intensity, cv::Mat depth)
: levels (1)
{
...
void RgbdImagePyramid::compute (const size_t num_levels)
{
    ...
    // if we already have some levels, we just need to compute the coarser
    // levels
    ...
    for (size_t idx = first; idx < num_levels; ++idx)
    {
        ...
        // pyrDownMeanSmoothIgnoreInvalid<float>(levels[idx - 1].depth,
        // levels[idx].depth);
...
void RgbdImage::calculateDerivativeX (const cv::Mat& img, cv::Mat& result)
{
    ...
    // cv::Sobel(img, result, -1, 1, 0, 3, 1.0f / 4.0f, 0,
    // cv::BORDER_REPLICATE);
...
void RgbdImage::calculateDerivativeY (const cv::Mat& img, cv::Mat& result)
{
    ...
    // cv::Sobel(img, result, -1, 0, 1, 3, 1.0f / 4.0f, 0,
    // cv::BORDER_REPLICATE);
...
void RgbdImage::warpIntensity (const AffineTransform& transformationd,
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x, ++idx, ++warped_intensity_ptr, ++warped_depth_ptr)
        {
            ...
            if (inImage (x_projected, y_projected))
            {
                ...
                *warped_intensity_ptr =
                Interpolation::bilinearWithDepthBuffer (this->intensity, this->depth,
                                                        x_projected, y_projected, z);
                *warped_depth_ptr = z;
            }
            else
            {
...
void RgbdImage::warpDepthForward (const AffineTransform& transformationx,
                                  const IntrinsicMatrix& intrinsics,
                                  RgbdImage& result,
                                  cv::Mat_<cv::Vec3d>& cloud)
{
    ...
    for (size_t y = 0; y < height; ++y)
    {
        for (size_t x = 0; x < width; ++x, ++depth_ptr)
        {
            ...
            Eigen::Vector3d p3d ((x - ox) * depth / intrinsics.fx (),
                                 (y - oy) * depth / intrinsics.fy (), depth);
            Eigen::Vector3d p3d_transformed = transformation * p3d;

            float x_projected =
            (float)(p3d_transformed (0) * intrinsics.fx () / p3d_transformed (2) + ox);
            float y_projected =
            (float)(p3d_transformed (1) * intrinsics.fy () / p3d_transformed (2) + oy);
...
void RgbdImage::warpIntensityForward (const AffineTransform& transformationx,
                                      const IntrinsicMatrix& intrinsics,
                                      RgbdImage& result,
                                      cv::Mat_<cv::Vec3d>& cloud)
{
        ...
        for (size_t x = 0; x < width; ++x, ++depth_ptr)
        {
            ...
            if (!identity)
            {
                ...
                if (inImage (x_projected, y_projected))
                {
                    ...
                    warped_image.at<IntensityType> (yp, xp) =
                    intensity.at<IntensityType> (y, x);
                }
                else
                {
                    outliers++;
    ...
    // std::cerr << "warp out: " << outliers << " total: " << total <<
    // std::endl;
...
void RgbdImage::warpDepthForwardAdvanced (const AffineTransform& transformation,
                                          const IntrinsicMatrix& intrinsics,
                                          RgbdImage& result)
{
    assert (hasDepth ());
    ...
    transformation.rotation () (0, 1) * (intrinsics.fx () / intrinsics.fy ());
    float x_factor1 =
    -transformation.rotation () (2, 0) -
    transformation.rotation () (2, 1) * (intrinsics.fx () / intrinsics.fy ());

    float z_factor2 =
    transformation.rotation () (1, 1) +
    transformation.rotation () (1, 0) * (intrinsics.fy () / intrinsics.fx ());
    float y_factor2 =
    -transformation.rotation () (2, 1) -
    transformation.rotation () (2, 0) * (intrinsics.fy () / intrinsics.fx ());
    ...
    for (int idx = 0; idx < height * width; ++idx)
    {
        ...
        int x_projected = (int)std::floor (
        p3d_transformed (0) * intrinsics.fx () / z_transformed + intrinsics.ox ());
        int y_projected = (int)std::floor (
        p3d_transformed (1) * intrinsics.fy () / z_transformed + intrinsics.oy ());

        // TODO: replace inImage(...) checks, with max(..., 0) on initial value
        // of x_, y_ and  min(..., width/height) for their respective upper
        // bound
