...
dvo::core::RgbdImagePyramid::Ptr load(std::string rgb_file,
                                      std::string depth_file)
{
    cv::Mat rgb, grey, grey_s16, depth, depth_inpainted, depth_mask, depth_mono,
        depth_float;
    ...
    if (depth.type() != CV_32FC1) {
        dvo::core::SurfacePyramid::convertRawDepthImageSse(depth, depth_float,
                                                           1.0f / 5000.0f);
    } else {
        depth_float = depth;
    }
...
bool BenchmarkNode::configure()
{
    ...
    if (nh_private_.getParam("rgbdpair_file", cfg_.RgbdPairFile)) {
        rgbdpair_reader_ = new dvo_benchmark::FileReader<dvo_benchmark::RgbdPair>(
            cfg_.RgbdPairFile);
    ...
    if (cfg_.EstimateTrajectory) {
        if (nh_private_.getParam("trajectory_file", cfg_.TrajectoryFile) && !cfg_.TrajectoryFile.empty()) {
            ...
            if (trajectory_out_->fail()) {
                ...
                std::cerr << "Failed to open '" << cfg_.TrajectoryFile << "'!"
                          << std::endl;
                return false;
            }
        } else {
    ...
    if (cfg_.ShowGroundtruth) {
        if (nh_private_.getParam("groundtruth_file", cfg_.GroundtruthFile)) {
            groundtruth_reader_ = new dvo_benchmark::FileReader<dvo_benchmark::Groundtruth>(
                cfg_.GroundtruthFile);
        ...
        if (dump_camera_pose_.value()) {
            if (!cfg_.CameraFile.empty()) {
                ...
                camera_pose_file << cams[0].clip[0] << "," << cams[0].clip[1] << "/"
                                 ...
                                 << cams[0].window_size[0] << ","
                                 << cams[0].window_size[1] << "/"
                                 << cams[0].window_pos[0] << ","
                                 << cams[0].window_pos[1];
...
void BenchmarkNode::createReferenceCamera(
    dvo::visualization::CameraTrajectoryVisualizerInterface* visualizer,
    const dvo::core::RgbdImage& img,
    const dvo::core::IntrinsicMatrix& intrinsics, const Eigen::Affine3d& pose)
{
...
void BenchmarkNode::run()
{
    ...
    if (cfg_.VisualizationRequired()) {
        visualizer = new dvo::visualization::PclCameraTrajectoryVisualizer(
            !cfg_.RenderVideo);
        ((dvo::visualization::PclCameraTrajectoryVisualizer*)visualizer)
    ...
    for (std::vector<dvo_benchmark::RgbdPair>::iterator it = pairs.begin();
         ros::ok() && it != pairs.end(); ++it) {
        reference = current;
        current = load(folder + it->RgbFile(), folder + it->DepthFile());

        if (!reference) {
            createReferenceCamera(visualizer, current->level(0), intrinsics,
                                  trajectory);
        }
        ...
        if (cfg_.ShowGroundtruth) {
            ...
            dvo_benchmark::toPoseEigen(groundtruth_reader_->entry(),
                                       groundtruth_pose);
        ...
        if (cfg_.EstimateRequired()) {
            ...
            if (cfg_.EstimateTrajectory) {
                ...
                (*trajectory_out_) << it->RgbTimestamp() << " "
